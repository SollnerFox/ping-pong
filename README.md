...
2. **Увімкни сервер** (хтось один)
   ```
   python server.py
   ```

3. **Запусти лаунчер (головне меню)**
   - Вхід у гру відбувається лише через лаунчер:
     ```
     python launcher.py
     ```
   - У меню натисни **«Грати»**, введи своє ім'я і натисни **«Почати гру»**.
   - Якщо граєте з іншого комп'ютера — вкажи IP сервера у **«Налаштуваннях»**.

> ⚠️ Усі файли мають бути у тій самій папці. Сервер запускається **лише один раз**, а `client.py` (через лаунчер) — для кожного гравця.
